...
export default {
	...
	transformTree(data, option = {}) {
		...
		if(_option.listArray) {
			...
			//          res = this.buildTree(res, data, _option.name, _option.keepParent)
		}
		if(option.pNode) {
	...
	getMenu(data) {
		...
		res = this.deepSort(res, 'order')
		return res
	},
	getValue(data, key = "id") {
		return data.map((val) => {
			return val[key]
		})
	},
	listToStr(data, key = "id") {
